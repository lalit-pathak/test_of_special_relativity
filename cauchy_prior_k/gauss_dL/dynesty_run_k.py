...
from utils import cdfinv_k, cdfinv, cdfinv_q

merger = Merger("GW170817")
ifos = ['L1', 'H1', 'V1'] # defining a list of interferometers
fLow = 20 # seismic cutoff frequency
whichfrac = 1.0 #float(input('fraction of fISCO: ')) # fraction of fISCO corresponding to M_map value (in Hz)
...
variable_params = ['mass1', 'mass2', 'spin1z', 'spin2z', 'k', 'inclination', 'distance', 'tc']
...
def pycbc_log_likelihood(query):
    ...
    mchirp, mass_ratio, s1z, s2z, k, iota, distance, tc = query
    m1 = mass1_from_mchirp_q(mchirp, mass_ratio)
    m2 = mass2_from_mchirp_q(mchirp, mass_ratio)
    
    model.update(mass1=m1, mass2=m2, spin1z=s1z, spin2z=s2z, k=k, inclination=iota, distance=distance, tc=tc)
...
def prior_transform(cube):
    ...
    cube[4] = cdfinv_k(k0, gamma, k_min, k_max, cube[4])
    cube[5] = np.arccos(2*cube[5] - 1) 
    cube[6] = cdfinv(distance_mean, distance_var, distance_min, distance_max, cube[6]) # distance: unifrom prior in dL**3
    cube[7] = tc_min + (tc_max - tc_min) * cube[7] # pol: uniform angle
...
file = h5py.File('samples_data_k_{}_fISCO_seedPE_{}_repeat.hdf5'.format(whichfrac, seed_PE), 'w')
...
file.create_dataset('tc', data=res['samples'][:,7])
