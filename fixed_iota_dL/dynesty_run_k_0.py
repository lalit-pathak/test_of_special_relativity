"""Script to run PE run using dynesty sampler for TaylorF2 model keeping distance and iota fixed to their MAP values
...
def pycbc_log_likelihood(query):
    ...
    mchirp, mass_ratio, s1z, s2z, tc = query
    m1 = mass1_from_mchirp_q(mchirp, mass_ratio)
    m2 = mass2_from_mchirp_q(mchirp, mass_ratio)
    
    model.update(mass1=m1, mass2=m2, spin1z=s1z, spin2z=s2z, tc=tc)
...
file = h5py.File('samples_data_{}_fISCO_seedPE_{}.hdf5'.format(whichfrac, seed_PE), 'w')
...
file.create_dataset('tc', data=res['samples'][:,4])
