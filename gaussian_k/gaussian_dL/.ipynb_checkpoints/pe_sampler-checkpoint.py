...
from rbf_pe_utils import RBFInterpolatedLikelihood, evaluateRBFList, cdfinv_q, prior_transform, pycbc_log_likelihood
...
def dynesty_sampler_pycbc(model, boundary, boundary_dL_tc, sampler_params=None, save_to_hdf=False):
    ...
    if(save_to_hdf):
        ...
        file = h5py.File('raw_samples_pycbc_seedPE_{}_nodes.hdf'.format(seed_PE), 'w')
        i = 0
        for key in sample_keys:
            file.create_dataset(key, data=result['samples'][:,i])
            i = i + 1
...
def generate_postSamps(raw_samples, save_to_hdf=False):
    ...
    if(save_to_hdf):
        ...
        file = h5py.File('post_samples_interp_seedPE_{}_nodes_seed_{}.hdf'.format(seed_PE, nodes_seed), 'w')   
        i = 0
        for key in params:
...
def generate_postSamps_pycbc(raw_samples, save_to_hdf=False):
    ...
    if(save_to_hdf):
        ...
        file = h5py.File('post_samples_pycbc_seedPE_{}.hdf'.format(seed_PE), 'w')   
        i = 0
        for key in params:
