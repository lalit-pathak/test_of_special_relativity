...
def quantile(x, q, weights=None, median=None):
    """
    ...
    if(median=='yes'):
        ...
        count, val = np.histogram(x, 25)
        val_pdf = kernel.pdf(val)
        map_val = val[np.argmax(val_pdf)]

        q_l, q_m, q_h = np.quantile(x, q)

        return np.array([q_l, map_val, q_h])
